...
- [x] `mvn test` aprovado em 07/05/2026 (pos separacao do modulo): 119 testes, 0 falhas.
- [x] `mvn verify -Pintegration` aprovado em 07/05/2026: 119 testes unitarios + 1 teste de integracao, 0 falhas.
...
- [ ] Importar `../PLANILHA_FISCAL.xlsm` para `operacao/empresas.yaml` definitivo e validar caminhos.
...
```text
# Importar TODOS os meses da planilha compartilhada de uma so vez:
...
# Modo batch automatico via script:
scripts/windows/rodar-batch-producao.bat --homologacao
```

O script `scripts/windows/rodar-batch-producao.bat` usa por padrao `..\PLANILHA_FISCAL.xlsm` e `operacao\empresas.yaml`.
...
java -jar target/renomeador-nfse-0.1.0-SNAPSHOT.jar batch --config operacao\empresas.yaml --homologacao --mes 2026-05
