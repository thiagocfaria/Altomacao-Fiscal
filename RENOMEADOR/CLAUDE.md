...
- Se a tarefa tocar em parser, extracao ou deteccao de layout, use a skill local `RENOMEADOR/.claude/skills/layout-nfse`.
- Se tocar em performance, watcher ou pipeline, use `RENOMEADOR/.claude/skills/validacao-performance-java` antes de declarar melhoria.

## Regras de prudencia

- Nao presumir MCP ativo so porque esta instalado na maquina.
- Nao presumir plugin Java ativo sem ver em `../.claude/settings.json` na raiz do projeto.
