...
- processamento sem copia tecnica duplicada de PDFs originais;
...
- planilha compartilhada `../PLANILHA_FISCAL.xlsm` para preencher `CAMINHO REST`;
...
- Validacao local de desenvolvimento em 07/05/2026: `mvn -Dmaven.repo.local=/tmp/m2-nfse test`: 119 testes, 0 falhas;
- Validacao local de integracao em 07/05/2026: `mvn -Dmaven.repo.local=/tmp/m2-nfse verify -Pintegration`: 119 testes unitarios + 1 teste de integracao, 0 falhas;
...
- `../PLANILHA_FISCAL.xlsm` disponivel na raiz do projeto para operacao/teste;
...
- importar a planilha compartilhada para `operacao/empresas.yaml`;
...
```text
C:\NFSE\ALTOMACAO-FISCAL\RENOMEADOR
```
...
scripts\windows\preparar-planilha.bat C:\entrada\PLANILHA_ORIGINAL.xlsm ..\PLANILHA_FISCAL.xlsm
scripts\windows\importar-planilha.bat ..\PLANILHA_FISCAL.xlsm operacao\empresas.yaml
scripts\windows\validar-config.bat operacao\empresas.yaml
...
A planilha de trabalho compartilhada fica na raiz em `../PLANILHA_FISCAL.xlsm`. Ela preserva o VBA da planilha original e agora e preparada em abas mensais de cadastro, `DASHBOARD` e `CONFIG`. O cadastro mantem filtros, cabecalho congelado, `CNPJ` e caminhos como texto, `CAMINHO REST`, `CAMINHO DMS`, `CAMINHO ENTRADA/SAIDA`, `CAMINHO CERTIFICADO DIGITAL`, `VALIDADE CERTIFICADO DIGITAL`, `SENHA CERTIFICADO DIGITAL` opcional e `SOMENTE ORIGEM` apenas para casos excepcionais de pasta generica com CNPJ invalido.
...
Mantenha apenas essa planilha modelo compartilhada na raiz do projeto. A planilha bruta/original serve somente como entrada para recriar o modelo quando chegar uma versao nova; planilhas fiscais extras com dados de clientes nao devem ser publicadas no GitHub.

O sistema importa a aba `CADASTRO` para `empresas.yaml`; em planilhas antigas, continua aceitando a aba `Dashboard Fiscal`. O `batch` e o `watch` rodam sobre esse YAML validado. Se `batch` ou `watch` forem chamados com `--planilha`, eles atualizam o `empresas.yaml` a partir da planilha antes de processar. Se `--planilha` nao for informado, mas existir `PLANILHA_FISCAL.xlsm` na pasta do `empresas.yaml`, na pasta do modulo ou na raiz compartilhada, essa planilha padrao tambem sera importada automaticamente. No `watch`, salvar a planilha informada, ou a planilha padrao encontrada automaticamente, faz o cadastro ser reimportado e os novos caminhos REST passam a ser observados.
...
O `DASHBOARD` nao e fonte de importacao: ele mostra a tela inicial da operacao contabil, com total de clientes, notas importadas hoje, XMLs importados hoje, certificados em alerta, pendencias de cadastro e os 3 certificados digitais mais proximos do vencimento. O card de certificados fica verde sem vencimentos proximos, amarelo quando houver vencimento em ate 30 dias e vermelho quando houver vencimento em menos de 15 dias. Ao atualizar a validade do certificado na linha do cliente e salvar a planilha, o painel recalcula e remove o alerta quando ele nao estiver mais proximo do vencimento.
...
O mesmo CNPJ de tomador nao pode aparecer em duas empresas de destino ativas no mesmo mes operacional. Essa validacao evita que uma nota em pasta errada seja roteada para a REST errada quando houver cadastro duplicado na planilha.
...
```bash
java -jar target/renomeador-nfse-0.1.0-SNAPSHOT.jar batch --config operacao/empresas.yaml
```

Execucao `batch` atualizando a partir da planilha antes da passada:

```bash
java -jar target/renomeador-nfse-0.1.0-SNAPSHOT.jar batch --config operacao/empresas.yaml --planilha ../PLANILHA_FISCAL.xlsm
```

Limitar a uma empresa e informar mes:

```bash
java -jar target/renomeador-nfse-0.1.0-SNAPSHOT.jar batch --config operacao/empresas.yaml --empresa empresa_piloto --mes 2026-04
```
...
```bash
java -jar target/renomeador-nfse-0.1.0-SNAPSHOT.jar watch --config operacao/empresas.yaml
```
